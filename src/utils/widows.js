...
//
// В редакторе его не бывает — последние два слова держит вместе правило
// переносов (useHangingWords), но держит декорацией: сам текст не меняется,
// и в файл это не уезжает. Типограф расставляет настоящие неразрывные
// пробелы, однако последнее слово связывает только короткое: «вышел вон» —
// да, «длинным словом впечатление» — нет.
//
// Здесь тот же порог, что и у живого правила, но настоящим неразрывным
// пробелом: применяется там же, где типограф, — по ⌘⇧T, в предпросмотре
// и в выгрузках. Так в файле выходит то же, что человек видел в редакторе.
